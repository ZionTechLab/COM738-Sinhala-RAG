...
const MODE_SHORT: Record<string, string> = {
  rag: 'RAG',
  baseline_a: 'A: Ungrounded',
  baseline_b: 'B: Constrained',
}
...
const COLLECTION_SHORT: Record<string, string> = {
  syllabus_paragraph_e5: 'Paragraphs',
  syllabus_section_e5: 'Sections',
  pastpaper_question_e5: 'Past Papers',
...
export function ParameterBar({ mode, setMode, collection, setCollection, topK, setTopK, model, setModel, availableModels, loading }: ParameterBarProps) {
  ...
  return (
    <>
      ...
      >
        <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" className="text-primary shrink-0">
        ...
        <div className="flex items-center gap-2 text-xs flex-wrap flex-1 min-w-0">
          <span className="text-text-muted whitespace-nowrap">{MODE_SHORT[mode] || mode}</span>
          <span className="text-card-border">·</span>
          <span className="text-text-muted whitespace-nowrap truncate">{modelName}</span>
          <span className="text-card-border">·</span>
          <span className="text-text-muted whitespace-nowrap">Top-{topK}</span>
          <span className="text-card-border">·</span>
          <span className="text-text-muted whitespace-nowrap truncate">{COLLECTION_SHORT[collection] || collection}</span>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-bold text-text-main">Parameters</h3>
            ...
            <div className="flex flex-col gap-4">
              {/* Mode */}
              <div>
                <label className="text-xs font-semibold text-text-muted uppercase tracking-wide mb-2 block">Execution Mode</label>
                <select value={mode} onChange={e => setMode(e.target.value as typeof mode)} disabled={loading}
                  className="bg-dark border border-card-border text-text-main p-2.5 rounded-lg font-sans text-sm w-full">
              ...
              {/* Model */}
              <div>
                <label className="text-xs font-semibold text-text-muted uppercase tracking-wide mb-2 block">LLM Model</label>
              ...
              {/* Collection */}
              <div>
                <label className="text-xs font-semibold text-text-muted uppercase tracking-wide mb-2 block">Collection</label>
              ...
              {/* Top-K */}
              <div>
                <label className="text-xs font-semibold text-text-muted uppercase tracking-wide mb-2 block">Top-K Chunks: {topK}</label>
                <input type="range" min={1} max={5} value={topK}
                  onChange={e => setTopK(Number(e.target.value))} disabled={loading}
